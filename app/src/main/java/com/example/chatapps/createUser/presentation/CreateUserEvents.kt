...
sealed interface CreateUserEvents{
    ...
    data class NavBackCountry(val navHostController: NavHostController, val code: String, val Flag: String): CreateUserEvents
    data class VerifyPhone(val context: Context, val phoneNumber: String): CreateUserEvents
    data class VerifyCredential(val context: Context, val navHostController: NavHostController): CreateUserEvents
}